...
import {
  createBooking,
  getAvailability,
} from "@/API/properties/singlePropertyApi";
...
export default function BookingPopup({
  ...
  const submitHandler = async (data: BookingFormValues) => {
    ...
    try {
      setLoading(true);
      const response = await createBooking(lang, payload);
  ...
  const handleCheckAvailability = async () => {
    ...
    try {
      ...
      const response = await getAvailability(lang, slug, companyId, payload);
      ...
      <div className="relative z-10 w-full max-w-4xl rounded-3xl bg-white shadow-2xl overflow-hidden flex flex-col md:flex-row animate-fadeIn max-h-[80vh]">
        ...
        <div className="md:w-1/2 bg-linear-to-b from-[#FFF8EE] to-[#FFF3E0] p-6 md:p-10 flex flex-col justify-between gap-2 md:gap-6">
          <div className="flex items-center justify-between">
            <h3 className="text-lg md:text-2xl font-bold text-(--primaryColor)">
              {t.bookingDetails}
            </h3>
            <button
              onClick={onClose}
          ...
          <div className="space-y-2 md:space-y-4">
            <div className="flex justify-between items-center bg-white/50 p-2 md:p-4 rounded-xl shadow-inner">
              ...
              <div>
                ...
                <p className="text-lg font-semibold">{propertyRooms}</p>
              </div>
            </div>

            <div className="grid grid-cols-2 gap-4">
              <div>
                <p className="text-sm text-gray-500">{`${t.selectCheckIn}`}</p>
                <DatePicker
                  selected={checkIn}
                  ...
                  minDate={today}
                  className=" bg-white/50 p-2 md:p-4 rounded-xl shadow-inner w-[100%]"
                  placeholderText={`${t.selectCheckIn}`}
                />
              </div>
              <div>
                <p className="text-sm text-gray-500">{`${t.selectCheckOut}`}</p>
                <DatePicker
                  selected={checkOut}
                  onChange={(date: Date | null) => {
                    setCheckOut(date);
          ...
          <button
            onClick={onClose}
            className=" mt-2 md:mt-6 self-start text-sm text-[#FF6B00] hover:underline transition"
          >
            {t.cancel}
          </button>
        </div>
