...
// import { FileText, CheckCircle, Bed, User } from "lucide-react";
...
import { FileText, CheckCircle, Bed, User } from "lucide-react";
import { getPropertyDetails, getGalleryImages } from "@/API/property";
import PropertyGallery from "@/component/singleProperty/PropertyGallery";
import BookingCardClient from "@/component/property/BookingCardClient";
import PropertyReviewsSection from "@/component/property/PropertyReviewsSection";
...
async function PropertyDetailPage({
  ...
  if (categories.length > 0) {
    categories.forEach((cat: string) => {
      const data = galleryImages?.[cat] || { images: [], videos: [] };

      formattedGallery[cat] = {
        images: (data.images || [])
          .filter(Boolean)
  ...
  if (Object.keys(formattedGallery).length === 0) {
    formattedGallery["Default"] = {
      images: [fixUrl(property.image_1920)],
      videos: [],
    };
  }
  ...
  return (
    <main className="w-full space-y-4">
      ...
      {/* MAIN GRID */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 px-4 md:px-20">
        {/* ✅ GALLERY */}
        <div className="col-span-12">
          <PropertyGallery
            fullGallery={formattedGallery}
            image_1920={property.image_1920}
            lang="en" // or dynamic if you have language
        ...
        <div className="col-span-12 lg:col-span-8 space-y-10">
          <div className="flex gap-6 text-gray-700 text-lg underline">
          ...
          <section className="space-y-4">
            {property.description ? (
              property.description.split("\n\n").map((p: string, i: number) => (
                <p
                  key={i}
                  className="text-black text-lg leading-relaxed md:text-2xl"
                >
                  {p}
                </p>
              ))
          ...
          <section>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {property.amenities?.map((amenity: Amenity) => (
                <div
                  key={amenity.id}
                  className="p-4 bg-white rounded-xl border shadow-sm flex items-center gap-3"
                >
                  <img
                    src={fixUrl(amenity.image)}
                    alt={amenity.name}
                    className="w-10 h-10 object-contain"
                  />
                  <span className="text-black md:text-2xl">{amenity.name}</span>
                </div>
              ))}
            </div>
          </section>
          ...
          <section>
            ...
            <ul className="space-y-3">
              {property.whats_include?.map((item: IncludedItem) => (
                <li
                  key={item.id}
                  className="flex items-center gap-3 md:text-2xl text-black"
                >
        ...
        <aside className="col-span-12 lg:col-span-4 space-y-6 lg:sticky lg:top-24">
          ...
          {/* POLICIES */}
          <div className="bg-[#f8f3ef] p-6 rounded-2xl border">
          ...
          {/* MAP */}
          <div className="rounded-xl overflow-hidden shadow border h-72">
            <iframe
              width="100%"
              height="100%"
              loading="lazy"
